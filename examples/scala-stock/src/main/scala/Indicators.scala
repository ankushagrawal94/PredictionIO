...
abstract class BaseIndicator extends Serializable {
	/** Calculates training series for a particular stock.
	  *
		* @param logPrice series of logarithm of all prices for a particular stock. Logarithm
		*                 is highly recommended for more accurate looking results.
		* @return the training series of the stock
		*/
  def getTraining(logPrice: Series[DateTime, Double]): Series[DateTime, Double]

	/** Applies indicator on a window size of the value returned by getMinWindowSize() and returns the
	  * last value in the resulting series to be used for prediction in RegressionStrategy.
	  *
...
/** Indicator that calcuate differences of opening/closing prices
  *
	...
	* @param period number of days between any 2 closing prices to consider for calculating a return
	*/
